    ...
    private void FixedUpdate()
    {
        ...
        if (IsGrounded && jumpInput && jumpPowerInternal > 0 && !didJump)
        {
            if (advanced.maxSlopeAngle > 0)
            {
                if (advanced.isTouchingFlat || advanced.isTouchingWalkable)
                {
                    ...
                    fps_Rigidbody.constraints = RigidbodyConstraints.None | RigidbodyConstraints.FreezeRotation;
                    // noise for when jumping (STEALTH)
        ...
        if (_crouchModifiers.useCrouch)
        {
            ...
            if (isCrouching)
            {
                ...
                // make less noise when crouched (STEALTH)
                noiseModifier = 0.0f;
            ...
            else
            {
                ...
                sprintSpeedInternal = sprintSpeed;
                jumpPowerInternal = jumpPower;
                // return to normal noise when not crouched (STEALTH)
                noiseModifier = 1.0f;
            }
        }

        // more noise when sprinting (STEALTH)
        if (isSprinting)
        {
            noiseModifier = 1.5f;
        }
...
public class FPAIO_Editor : Editor
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        EditorGUILayout.Space();
        EditorGUILayout.Space();
        // WIP for debug(STEALTH)
        t.noiseModifier = EditorGUILayout.Slider(new GUIContent("Current Noise Modifier", "Determines The Amount Of Noise made by actions."), t.noiseModifier, 0.0f, 5.0f);
        EditorGUILayout.Space();
        EditorGUILayout.Space();
